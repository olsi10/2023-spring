...
public class ValidationController {
    ...
    */
    ...
    @PostMapping
    public ResponseEntity<?> addMember(@Valid @RequestBody Member m, BindingResult
            result) {
// 에러가 발생했는지 여부 확인
        if(result.hasErrors()) {
// 발생한 에러 개수 출력
            ...
            return
                    ResponseEntity.badRequest().body(fieldErrorListToHashMap(result.getFieldErrors()));
        }
// 실제 회원 가입 로직 처리하는 부분 (ex: 데이터베이스에 회원 데이터 삽입)
// ... code here ...
        return ResponseEntity.ok().body(m);
    }
    // 에러가 발생한 모든 필드 순회하여 HashMap 객체로 변경하는 메서드
// HashMap은 Jackson 메시지 컨버터가 JSON으로 변환 가능
